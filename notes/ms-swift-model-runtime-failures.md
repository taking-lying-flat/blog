...
<table>
  ...
  <tbody>
    <tr><td><code>Config resolution</code></td><td>模型类型、子模块开关、<code>RoPE</code> 和输出模态配置</td><td>配置只能创建 <code>checkpoint</code> 实际支持的模块拓扑</td></tr>
    <tr><td><code>Construction / post_init</code></td><td><code>Parameter</code> 与由公式生成的 <code>buffer</code></td><td>构造结束后的所有运行时状态均已确定且可复现</td></tr>
    <tr><td><code>Checkpoint loading</code></td><td>持久 <code>Parameter</code> 与 <code>persistent buffer</code></td><td>不能假设 <code>checkpoint</code> 会恢复 <code>persistent=False</code> 状态</td></tr>
    <tr><td><code>Collation</code></td><td><code>Padding</code>、<code>packing</code>、<code>M-RoPE</code>、<code>sequence boundary</code></td><td><code>tensor token</code> 数与 <code>metadata</code> 描述的 <code>token</code> 数一致</td></tr>
    <tr><td><code>Kernel execution</code></td><td><code>Q/K/V</code>、<code>GDN state</code>、<code>workspace</code>、<code>cu_seqlens</code></td><td><code>shape</code>、<code>layout</code>、<code>dtype</code> 与 <code>offset</code> 必须闭合</td></tr>
    <tr><td><code>Generation</code></td><td>停止 <code>token</code> 与 <code>decode state</code></td><td>调用哪一层 <code>generate()</code>，就由哪一层负责终止参数</td></tr>
  </tbody>
</table>
</div>
