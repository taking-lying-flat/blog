...
有效 response_prefix is None
    → 样本级与全局均未显式指定前缀
    → 根据解析后的 enable_thinking 选择模板前缀：
        thinking     → template_meta.thinking_prefix
        non-thinking → template_meta.non_thinking_prefix

有效 response_prefix is not None
    → 已显式指定前缀（包括空字符串 ''）
    → 直接使用 response_prefix
    → 不再使用 template_meta.thinking_prefix / non_thinking_prefix
```
